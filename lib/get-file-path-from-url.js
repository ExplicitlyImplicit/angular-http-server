 ...
 */
function getFilePathFromUrl(url, basePath, { pathLib = path, publicPath = '' } = {}) {
  if (!basePath) {
  ...
  if (url.length < publicPath.length || !url.toLowerCase().startsWith(publicPath.toLowerCase())) {
    // public path must be in the url
    return 'dummy';
  }

  const relativePath = url.substr(publicPath.length).split('?')[0];
